...
function matchCommentReply(commentText, senderId) {
  const config = loadConfig();
  const normalized = (commentText || '').trim().toLowerCase();

  let { id, entry, isTreatment } = findConcreteReply(config, normalized);

  if (!entry && config.generalCommentKeywords && keywordsMatch(config.generalCommentKeywords, normalized)) {
    id = 'general-comment-interest';
    entry = config.multiTreatmentReply;
    isTreatment = false;
  }
